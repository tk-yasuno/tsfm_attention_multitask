"""
Forecast Visualization for v2.2 Multi-Task Model
v2.2マルチタスクモデルの予測可視化
...
class MultiTaskHybridModel(nn.Module):
    ...
    def __init__(
        ...
        self.fusion_layer = nn.Sequential(
            nn.Linear(self.embed_dim + stat_feature_dim, hidden_dim),
    ...
    def forward(self, sequence: torch.Tensor, features: torch.Tensor, return_attention: bool = False):
        with torch.no_grad():
            if self.is_lstm:
                # LSTMの場合
        ...
        fused = torch.cat([attended_emb, features], dim=1)
        shared_hidden = self.fusion_layer(fused)
...
class ForecastVisualizerV2_2:
    ...
    def load_model(self):
        """モデル読み込み"""
        print(f"Loading v2.2 Multi-Task Model...")
    ...
    def predict_sample(
        ...
        """
        # Tensor変換
        sequence_tensor = torch.FloatTensor(sequence).unsqueeze(0).unsqueeze(-1).to(self.device)  # [1, 90, 1]
        
        # 特徴量の数値型変換
        feature_values = pd.to_numeric(pd.Series(feature_values), errors='coerce').values
        feature_values = np.nan_to_num(feature_values, nan=0.0, posinf=0.0, neginf=0.0)
        
        feature_tensor = torch.FloatTensor(feature_values).unsqueeze(0).to(self.device)  # [1, 28]
        
        # 予測
        with torch.no_grad():
            predictions, attention_weights = self.model(sequence_tensor, feature_tensor, return_attention=True)
        ...
        attention_weights = attention_weights.cpu().numpy()[0]  # [num_heads, 1, 1]
    ...
    def generate_nonlinear_forecast(
        ...
        trend_slope = features.get('feat_trend_slope', 0.0)
        ...
        # トレンド調整
        adjusted_forecast += trend_slope * np.arange(num_points) * 0.1
        
        # 物理的制約（±3σ）
        mean_val = features.get('feat_mean', np.mean(sequence))
        std_val = features.get('feat_std', np.std(sequence))
        adjusted_forecast = np.clip(adjusted_forecast, mean_val - 3*std_val, mean_val + 3*std_val)
    ...
    def plot_multitask_comparison(
        ...
        for idx, sample_idx in enumerate(sample_indices):
            ...
            features_dict = {col: float(val) for col, val in zip(self.feature_cols, feature_values)}
            
            # 予測
            predictions, pred_labels, attention_weights = self.predict_sample(sequence, feature_values)
            ...
            for h_name, pred_prob, pred_label, actual_label, h_days, ax in horizons:
                ...
                # 非線形予測
                forecast = self.generate_nonlinear_forecast(sequence, features_dict, h_days, num_points=30)
        ...
        save_path = Path(RESULTS_ROOT) / f"forecast_comparison_v2.2_{timestamp}.png"
    ...
    def _select_diverse_samples(self, test_df: pd.DataFrame, num_samples: int) -> List[int]:
        ...
        for idx in range(len(test_df)):
            ...
            predictions, pred_labels, _ = self.predict_sample(sequence, feature_values)
...
def main():
    """メイン実行"""
    print("="*80)
    print("v2.2 Multi-Task Model Forecast Visualization")
    print("="*80)
    ...
    # モデル読み込み
    model_path = Path(MODEL_ROOT) / "hybrid_model_v2.2" / "pytorch_model_multitask.pt"
    
    if not model_path.exists():
        print(f"\n❌ Model not found: {model_path}")
        print("Please train the model first using train_hybrid_model_v2_2.py")
        return
    
    visualizer = ForecastVisualizerV2_2(model_path, device)
    ...
    visualizer.feature_cols = [col for col in df_test.columns if col not in exclude_cols]
