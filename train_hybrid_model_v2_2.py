"""
Hybrid Model v2.2: Multi-Task Learning with Temporal Attention
ハイブリッドモデルv2.2 - マルチタスク学習と時間的注意機構

v2.1からの改善点:
1. Temporal Attention機構
   - 過去90日の重要な時点を自動選択
   - Multi-Head Self-Attention
   - 季節性・周期性の明示的モデル化

2. Multi-Task Learning
   - 3つのホライズン（30d、60d、90d）を同時学習
   - 共通特徴の効率的抽出
   - パラメータ数削減とパフォーマンス向上

3. 共有Encoder + 個別Classifier
   - Encoder + Attention: 共有（すべてのホライズンで共通知識）
   - Classifier: 個別（各ホライズン特有の予測）
...
class FocalLoss(nn.Module):
    ...
    v2.2改善: マルチタスク対応（複数ホライズンの損失を同時計算）
    """
...
class MultiTaskDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        return {
            'sequence': sequence,
            'features': features,
            'labels': labels  # マルチタスク
        }


class MultiTaskHybridModel(nn.Module):
    """
    v2.2 Multi-Task Hybrid Model with Temporal Attention
    ...
    3. Feature Fusion
       - Attention出力 [64] + 統計特徴 [28] = [92]
    
    4. Shared Hidden Layer
       - [92] → [128] (共通特徴抽出)
    
    5. Multi-Task Heads (3つの独立Classifier)
       - 30d Head: [128] → [1]
       - 60d Head: [128] → [1]
       - 90d Head: [128] → [1]
    """
    ...
    def forward(
        ...
        # Feature Fusion
        fused = torch.cat([attended_emb, features], dim=1)  # [batch, 92]
        shared_hidden = self.fusion_layer(fused)  # [batch, 128]
...
class MultiTaskTrainer:
    ...
    def train_epoch(self) -> Dict[str, float]:
        ...
        for batch in self.train_loader:
            ...
            # Forward
            predictions = self.model(sequences, features)  # [batch, 3]
    ...
    def train(self, epochs: int = 25, save_dir: Path = None) -> Dict:
        ...
        """
        if save_dir is None:
            save_dir = Path(MODEL_ROOT) / "hybrid_model_v2.2"
        ...
        print("\n" + "="*80)
        print("Starting Multi-Task Training (v2.2)")
        print("="*80)
...
def main():
    """メイン実行"""
    print("="*80)
    print("Hybrid Model v2.2 Training: Multi-Task Learning with Temporal Attention")
    print("="*80)
    ...
    # 履歴保存
    history_path = Path(RESULTS_ROOT) / "training_history_v2.2.json"
